...
func cmdAdd(args *skel.CmdArgs) error {
	...
	if cfg.IPAM.Type != "" {
		...
		// 根据IP，反向查询其mac地址
		//Note: 只支持单张弹性网卡直通
		macAddr, err := getLinkMacAddr(args)
		if err != nil {
			return err
		}
		if macAddr == nil {
			return errors.New("can't find container interface mac address")
		}

		hostDev, err := getLink(cfg.Device, *macAddr, cfg.KernelPath, cfg.PCIAddr)
		if err != nil {
...
func cmdDel(args *skel.CmdArgs) error {
	...
	log.Printf("cmdDel: load conf by %s", args.ContainerID)
	if args.Netns == "" {
		//Note: ensure to execute ipam cmdDel
	...
	if err := moveLinkOut(containerNs, args.IfName); err != nil {
		log.Printf("move link out err:%v", err.Error())
		return err
	}
...
func getLinkMacAddr(args *skel.CmdArgs) (*string, error) {
	ipamConf, _, err := allocator.LoadIPAMConfig(args.StdinData, args.Args)
	if err != nil {
		return nil, err
	}
	networkClient, err := neutron.GetNetworkClient(ipamConf)
	if err != nil {
		return nil, err
	}

	port, err := neutron.FindPort(networkClient, args.ContainerID)
	if err != nil {
		return nil, err
	}
	if port == nil {
		return nil, fmt.Errorf("can't find neutron port with name %s", args.ContainerID)
	}
	return &port.MACAddress, nil
}
