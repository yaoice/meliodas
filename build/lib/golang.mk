...
GO_LDFLAGS += -s -w -X ${VERSION_PACKAGE}.GitVersion=${VERSION} -X ${VERSION_PACKAGE}.GitCommit=${COMMIT} -X ${VERSION_PACKAGE}.BuildDate=$(shell date -u +'%Y-%m-%dT%H:%M:%SZ')
...
go.build.%:
	...
	@CGO_ENABLED=0 GOOS=$(OS) GOARCH=$(ARCH) $(GO) build -o $(OUTPUT_DIR)/$(OS)/$(ARCH)/$(COMMAND)$(GO_OUT_EXT) -ldflags "$(GO_LDFLAGS)" ./cmd/$(COMMAND)
